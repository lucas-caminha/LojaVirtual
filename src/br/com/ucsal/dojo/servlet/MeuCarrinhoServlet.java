...
public class MeuCarrinhoServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		session.setAttribute("carrinho", Carrinho.getProdutosNoCarrinho().values());
		session.setAttribute("qtdProdutos", Carrinho.getProdutosNoCarrinho().size());
		
		String contexto = req.getContextPath();
		
		resp.sendRedirect(contexto + "/meuCarrinho.jsp");
	}
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Carrinho.removeDoCarrinho(produto);
		Oferta.adicionaOferta(produto);
